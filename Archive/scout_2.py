...
class AllPositionsCleaning:
    ...
    def _create_master_consolidated_df(self):
        ...
        agg_rules = {
            ...
            'Age': 'max', 'Pos': 'first', 'Squad': 'last', 'Nation': 'first', # Must keep Nation
        ...
        # --- THE NATION CLEANING & AFCON FIX (IMPLEMENTATION) ---
        ...
        # C. Calculate Adjusted Denominator
    ...
    ###To reward defensive midfielders
    def calculate_defensive_multiplier(self, tkl, intel, mins):
        """Calculates a boost based on Defensive Actions (Tackles + Interceptions) per 90."""
        # Defensive measure, although filtering of >= 800 min has already been done
        if pd.isna(mins) or mins < 200:
             return 1.0
            
        # Calculate Defensive Actions per 90 (DA p90)
        da_p90 = ((tkl + intel) / mins) * 90
        
        # 20% Boost for Elite (10+)
        if da_p90 >= 10: 
            return 1.20  
        # 10% Boost for Above Average (8+)
        elif da_p90 >= 8: 
            return 1.10  
        # 5% Boost for Good (6+)
        elif da_p90 >= 6: 
    ...
    def search_player(self, name_query):
        ...
        cols = ['Player', 'Nation', 'Pos', 'Squad', 'Min', 'Min%', 'Weekly Wages', 'Age']
...
class Defender(AllPositionsCleaning):
    ...
    def get_defender_report(self):
        ...
        df_def['Base_Score'] = (
            ...
            (5 * df_def['Int'].fillna(0)) + 
            (50 * df_def['Gls'].fillna(0)) + 
            (30 * df_def['Ast'].fillna(0)) + 
...
class Midfielder(AllPositionsCleaning):
    ...
    def get_midfielder_report(self):
        ...
        # Performance Score Calculation
        df_mf['Base_Score'] = (
            (30 * df_mf['Ast'].fillna(0)) + 
            (50 * df_mf['Gls'].fillna(0)) + 
        ...
        df_mf['Def_Multiplier'] = df_mf.apply(
            lambda x: self.calculate_defensive_multiplier(x['TklW'], x['Int'], x['Min']), 
            axis=1
        )
        df_mf['Final_Score'] = (
            df_mf['Base_Score'] * df_mf['Availability_Multiplier'] * df_mf['Age_Multiplier'] * df_mf['Def_Multiplier']
        )
