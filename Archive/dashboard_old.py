...
# Call with your local filename (e.g., 'stadium.jpg')
set_bg_from_local("background.jpg")



###Newest with draft team
...
st.sidebar.markdown("---")
# Added "Value Graphs" to the navigation
...
if page == "Search Player":
    ...
    if target_display:
        ...
        # SAFETY CHECK: Prevent IndexError if player is missing/cached incorrectly
...
# --- 6. VALUE GRAPHS PAGE (NEW) ---
    ...
    # Define colors: Green for bargains (Value_Gap > 0), Red for underperformers
    ...
    # 2. Add Fair Value Regression Line
...
# --- 7. LEADERBOARDS PAGE ---
    ...
    for pos_choice, pos_code in mapping.items():
        ...
        # Display with Professional Column Formatting
        st.dataframe(
            df_top[cols_order], 
            hide_index=True, 
            ...
            column_config={
                "Weekly Wages": st.column_config.NumberColumn(
                    ...
                    format="£%,d", # The comma here adds the thousands separator
                ),
...
# --- 8. DRAFT A TEAM PAGE ---
    ...
    if formation_choice == "4-4-2":
        ...
        # Specialized 4-2-3-1 Elite Baseline for the ceiling
        mf_p = position_reports['MF']
        # Elite DMs (Prioritizing 1.2 Specialists but allowing 1.1 Elites)
    ...
    if wage_limit:
        ...
        if st.button("Generate Optimized XI"):
            ...
            for pos in ["GK", "DF", "MF", "FW"]:
                ...
                # Apply 15% Specialist Priority for 4-2-3-1 DMs
            ...
            st.dataframe(final_df[['Player', 'Pos', 'Squad', 'Weekly Wages', 'Final_Score']], use_container_width=True, hide_index=True)
